...
void
context::resume_( detail::data_t & d) noexcept {
    boost::context::continuation c = boost::context::callcc( std::move( c_), & d);
    detail::data_t * dp = boost::context::data< detail::data_t * >( c);
    if ( nullptr != dp) {
...
context::suspend_with_cc() noexcept {
    ...
    detail::data_t d{ prev };
    // context switch
    return boost::context::callcc( std::move( c_), & d);
}
